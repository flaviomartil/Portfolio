...
public function sendEmail(Request $request){
  ...
  if(!$mensagem == null && !$remetente == null && !$assunto == null && !$nome==null){
  $data = array('name'=>$nome,"assunto"=>$assunto,"remetente"=>$remetente,"mensagem"=>$mensagem);

  Mail::send(['text'=>'email'], $data, function($message) {
     $message->to('flaviomartil5@gmail.com', 'Flavio Martil')->subject
        ('Email do portfolio');
     $message->from('muxibashop@gmail.com','Contato Portfolio');
  });
  echo "E-mail enviado, em breve responderemos seu contato.";
}else{
  echo "Preencha todos os dados para enviar o e-mail por favor";
}
}
}
